...
export function ColorPaletteDisplay({ state }: { state: "show" | "hide" }) {
  ...
  return (
    <ColorPaletteBox state={state}>
      {colorPalette && (
        <>
          <div className="px-4 py-3 grid @xl:flex gap-4 justify-between items-center border-b bg-sidebar sticky top-0 z-10">
...
const ColorPaletteBox = ({
  ...
  return (
    <div
      ...
      data-state={state}
    >
      <section className="bg-background rounded-t-3xl md:rounded-3xl lg:rounded-r-none lg:h-[calc(100vh-88px-28px)] lg:max-h-[calc(100vh-88px-28px)] overflow-y-auto relative">
        {children}
      </section>
      <div className="hidden lg:block w-full h-[calc(100%-24px)] -z-10 absolute top-2 left-6 bg-[#50b1d8]/50 rounded-3xl rounded-r-none blur-xs" />
    </div>
  )
}
